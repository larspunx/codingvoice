#!/bin/sh
# Capture agent response text into the Coding Voice queue (Cursor hook: afterAgentResponse).
set -eu
...
if [ ! -f "$HOOK" ]; then
  mkdir -p "$(dirname "$LOG")"
  date -u "+[%Y-%m-%dT%H:%M:%SZ] capture: missing ${HOOK}" >> "$LOG" 2>/dev/null || true
  exit 0
fi
